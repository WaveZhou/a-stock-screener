...
<body>
    <div class="container">
        ...
        <div class="stats-bar">
            ...
            <div class="stat-card"><h3>日均涨幅</h3><div class="value" id="avgDay">-</div></div>
            <div class="stat-card"><h3>周均涨幅</h3><div class="value" id="avgWeek">-</div></div>
            <div class="stat-card"><h3>月均涨幅</h3><div class="value" id="avgMonth">-</div></div>
            <div class="stat-card"><h3>年均涨幅</h3><div class="value" id="avgYear">-</div></div>
        </div>
        <div class="btn-bar">
    ...
    <script>
        ...
        function renderDesktopTable(stocks) {
            var h = '<table><thead><tr><th>#</th><th>代码</th><th>名称</th><th>最新价</th><th>涨跌幅</th><th>总市值(亿)</th><th>净利润增速</th><th>周涨幅</th><th>月涨幅</th><th>年涨幅</th><th>成交额(万)</th><th>操作</th></tr></thead><tbody>';
            for (var i = 0; i < stocks.length; i++) {
                var s = stocks[i];
                var cc = s['涨跌幅'] > 0 ? 'positive' : s['涨跌幅'] < 0 ? 'negative' : '';
                var gc = (s['净利润同比增长率']||0) > 0 ? 'positive' : (s['净利润同比增长率']||0) < 0 ? 'negative' : '';
                var wc = (s['周涨幅']||0) > 0 ? 'positive' : (s['周涨幅']||0) < 0 ? 'negative' : '';
                var mc = (s['月涨幅']||0) > 0 ? 'positive' : (s['月涨幅']||0) < 0 ? 'negative' : '';
                var yc = (s['年涨幅']||0) > 0 ? 'positive' : (s['年涨幅']||0) < 0 ? 'negative' : '';
                ...
                h += '<tr>'
                    ...
                    + '<td class="' + wc + '">' + fmt(s['周涨幅'],2) + '%</td>'
                    + '<td class="' + mc + '">' + fmt(s['月涨幅'],2) + '%</td>'
                    + '<td class="' + yc + '">' + fmt(s['年涨幅'],2) + '%</td>'
        ...
        function renderMobileCards(stocks) {
            ...
            for (var i = 0; i < stocks.length; i++) {
                ...
                h += '<div class="stock-card">'
                    ...
                    +   '<div class="card-field"><span class="label">周涨幅</span><span class="val ' + ((s['周涨幅']||0)>0?'positive':(s['周涨幅']||0)<0?'negative':'') + '">' + fmt(s['周涨幅'],2) + '%</span></div>'
                    +   '<div class="card-field"><span class="label">月涨幅</span><span class="val ' + ((s['月涨幅']||0)>0?'positive':(s['月涨幅']||0)<0?'negative':'') + '">' + fmt(s['月涨幅'],2) + '%</span></div>'
                    +   '<div class="card-field"><span class="label">年涨幅</span><span class="val ' + ((s['年涨幅']||0)>0?'positive':(s['年涨幅']||0)<0?'negative':'') + '">' + fmt(s['年涨幅'],2) + '%</span></div>'
                    + '</div>'
        ...
        function updateStats(stocks) {
            ...
            var tc = 0, tg = 0, td = 0, tw = 0, tm = 0, ty = 0;
            for (var i = 0; i < stocks.length; i++) {
                tc += (stocks[i]['总市值'] || 0);
            ...
            var n = stocks.length;
            document.getElementById('avgCap').textContent = (tc / n / 1e8).toFixed(1) + '亿';
            document.getElementById('avgGrowth').textContent = (tg / n).toFixed(1) + '%';
            document.getElementById('avgDay').textContent = (td / n).toFixed(2) + '%';
            document.getElementById('avgWeek').textContent = (tw / n).toFixed(2) + '%';
            document.getElementById('avgMonth').textContent = (tm / n).toFixed(2) + '%';
            document.getElementById('avgYear').textContent = (ty / n).toFixed(2) + '%';
        }
        ...
        function exportExcel() {
            ...
            var d=[['排名','代码','名称','最新价','涨跌幅(%)','总市值(亿)','流通市值(亿)','净利润增速(%)','周涨幅(%)','月涨幅(%)','年涨幅(%)','成交额(万)','今开','最高','最低','昨收','换手率(%)']];
            for(var i=0;i<currentStocks.length;i++){
                ...
                d.push([i+1,s['代码'],s['名称'],s['最新价']||0,s['涨跌幅']||0,
                    ...
                    s['净利润同比增长率']||0,s['周涨幅']||0,s['月涨幅']||0,s['年涨幅']||0,
                    +((s['成交额']||0)/1e4).toFixed(2),
                    s['今开']||0,s['最高']||0,s['最低']||0,s['昨收']||0,s['换手率']||0]);
            }
            var wb=XLSX.utils.book_new(),ws=XLSX.utils.aoa_to_sheet(d);
            ws['!cols']=[{wch:5},{wch:10},{wch:12},{wch:8},{wch:10},{wch:11},{wch:11},{wch:12},{wch:10},{wch:10},{wch:10},{wch:12},{wch:8},{wch:8},{wch:8},{wch:8},{wch:10}];
